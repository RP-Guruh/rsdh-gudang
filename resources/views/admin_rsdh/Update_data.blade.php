...
<div class="container w-full mx-auto pt-20">
    <div class="w-full px-4 md:px-0 md:mt-8 mb-16 text-gray-800 leading-normal">
        <div class="mx-4 card bg-white max-w-md p-10 md:rounded-lg my-8">
            <div class="title">
                <h1 class="font-bold text-center">UPDATE DATA BARANG</h1>
            </div>
            <div class="form mt-4">
                <form action="{{ url('admin/rsdh/update/barang/' . $barang->kode_barang) }}" method="post"
                    ...
                    <div class="flex flex-col text-sm">
                        ...
                        <input name="nama_barang"
                            ...
                            type="text" placeholder="Nama Barang" value="{{ $barang->nama_barang }}">
                    </div>
                    <div class="options md:flex md:space-x-6 text-sm items-center text-gray-700 mt-4">
                        ...
                        <select name="kategori"
                            ...
                            <option value="Bahan Terolah"
                                {{ $barang->kategori == 'Bahan Terolah' ? 'selected' : '' }}>Bahan Terolah</option>
                            <option value="Bahan Segar" {{ $barang->kategori == 'Bahan Segar' ? 'selected' : '' }}>
                                Bahan Segar</option>
                            <option value="Bahan Siap Saji"
                                {{ $barang->kategori == 'Bahan Siap Saji' ? 'selected' : '' }}>Bahan Siap Saji
                            </option>
                        </select>
                    </div>
                    <div class="flex flex-col text-sm mt-4">
                        ...
                        <input name="harga_min"
                            class=" appearance-none border border-gray-200 p-2 focus:outline-none focus:border-gray-500"
                            type="number" placeholder="Harga Minimum (Rupiah)" value="{{ $barang->harga_min }}">
                    </div>
                    <div class="flex flex-col text-sm mt-4">
                        <label for="title" class="font-bold mb-2">Harga Maximum</label>
                        <input name="harga_max"
                            class=" appearance-none border border-gray-200 p-2 focus:outline-none focus:border-gray-500"
                            type="number" placeholder="Harga Maximum (Rupiah)" value="{{ $barang->harga_max }}">
                    </div>
                    <div class="options md:flex md:space-x-6 text-sm items-center text-gray-700 mt-4">
                        ...
                        <select name="satuan_hitung"
                            ...
                            <option value="Per-Liter" {{ $barang->satuan_hitung == 'Per-Liter' ? 'selected' : '' }}>
                                Per-Liter</option>
                            <option value="Per-Kilogram"
                                {{ $barang->satuan_hitung == 'Per-Kilogram' ? 'selected' : '' }}>
                                Per-Kilogram</option>
                            <option value="Per-Sachet" {{ $barang->satuan_hitung == 'Per-Sachet' ? 'selected' : '' }}>
                                Per-Sachet </option>
                        </select>
                    </div>
                    <div class="flex flex-col text-sm mt-4">
                        ...
                        <input name="exp_date"
                            class=" appearance-none border border-gray-200 p-2 focus:outline-none focus:border-gray-500"
                            type="date" placeholder="Tanggal Pesan" value="{{ $barang->exp_date }}">
                    </div>
            </div>
            <div class="submit">
                <button type="submit"
                    class=" w-full bg-blue-600 shadow-lg text-white px-4 py-2 hover:bg-blue-700 mt-8 text-center font-semibold focus:outline-none ">Update</button>
            </div>
            </form>
            <div class="mt-4">
